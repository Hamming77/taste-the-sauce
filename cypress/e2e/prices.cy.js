...
it('confirms the item with the lowest price', () => {
  ...
  // Tip 2: Cypress._.map is really universal => Creates an array of values by running each element in collection
    .then($priceElementsList => Cypress._.map($priceElementsList, 'innerText'))
    .then(console.log)
  // each price string has "$" character in front
  // remove it using string "substr" method
    .then((priceLiteralsList) => priceLiteralsList.map(s => s.slice(1)))
    .then(console.log)
  // convert each price string into a Number
    .then((priceStringsList) => priceStringsList.map(Number))
    .then(console.log)
  // find the smallest price number using Cypress._.min
  // and confirm it is 7.99
    .then($priceNumbersList => Cypress._.min($priceNumbersList))
    .should('equal', 7.99)
})
